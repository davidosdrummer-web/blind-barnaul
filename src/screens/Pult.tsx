...
import {
  ...
  Coins, Users, Zap, Repeat, Sparkles, Trophy, Hourglass, AlertTriangle,
} from "lucide-react";
import { useFirebaseData } from "../lib/useFirebaseData";
import { useAuth } from "../lib/useAuth";
import {
  Tournament, fmtClock, fmtNum, levelInfo, nextLevelOf, bankChips, lateRegOpen,
  ReturnMethod, plural, returnChipsFor,  // <-- returnChipsFor теперь из db.ts
} from "../lib/db";
import {
...
const RETURN_LABELS: Record<ReturnMethod, string> = { 
  rebuy: "Ребай", 
  reentry: "Ре-энтри", 
  addon: "Адд-он", 
  last_chance: "Ласт Шанс" 
};
...
export default function Pult({ preselect }: { preselect?: string }) {
    ...
    return (
      <div className="panel">
        ...
        <div className="flex justify-center pb-10">
          <Btn onClick={() => nav("/app/tournaments")}><Trophy className="size-4" /> К турнирам</Btn>
        </div>
      </div>
    );
  }
  ...
  const doKnock = async () => {
    ...
    setLoading(true);
    try {
      const err = await eliminate(t.id, outUid, byUid || null);
      if (err) toast(err, "err");
      else { toast("Игрок выбыл из турнира"); setModal(null); setOutUid(""); setByUid(""); }
    } catch (err: any) {
  ...
  const doBonus = async () => {
    if (!bonusId || !bonusUid) { toast("Выберите бонус и участника", "err"); return; }
    setLoading(true);
    try {
      const err = await giveBonus(t.id, bonusId, bonusUid);
      if (err) toast(err, "err");
      else { toast("Бонус начислен"); setModal(null); }
    } catch (err: any) {
      toast(err.message || "Ошибка", "err");
    } finally {
      setLoading(false);
  ...
  const doReturn = async (uidv: string, m: ReturnMethod) => {
    setLoading(true);
    try {
      const err = await returnPlayer(t.id, uidv, m);
      if (err) toast(err, "err");
      else toast(`${RETURN_LABELS[m]}: игрок вернулся в игру`);
    } catch (err: any) {
      toast(err.message || "Ошибка", "err");
    } finally {
      setLoading(false);
  ...
  const doLastChance = async (uidv: string) => {
    const chips = +lcChips || 0;
    if (chips <= 0) { toast("Укажите количество фишек для Ласт Шанс", "err"); return; }
    setLoading(true);
    try {
      const err = await returnPlayer(t.id, uidv, "last_chance", chips);
  ...
  const doWithdraw = async () => {
    ...
    setLoading(true);
    try {
      const err = await withdrawChips(t.id, amount);
      if (err) { toast(err, "err"); return; }
  ...
  const doFinish = async () => {
    ...
    try {
      ...
      setModal(null);
      toast("Турнир завершён — итоги подведены, очки начислены");
      nav("/app/tournaments");
    } catch (err: any) {
      toast(err.message || "Ошибка", "err");
    } finally {
  ...
  const handleTogglePause = async () => {
    try {
      await togglePause(t.id);
    } catch (err: any) {
      toast(err.message || "Ошибка", "err");
    }
  };

  const handleStepLevel = async (dir: 1 | -1) => {
    try {
      await stepLevel(t.id, dir);
    } catch (err: any) {
      toast(err.message || "Ошибка", "err");
    }
  };
    ...
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select 
            className="inp !min-h-[42px] !w-auto !py-2 text-[13px] font-bold" 
            value={t.id} 
            ...
            <div className="relative p-5 sm:p-7">
              ...
              <div className="mt-4 flex flex-col items-center gap-5 sm:flex-row sm:items-center sm:justify-around">
                <div className="text-center">
                  <p className={cn("num text-[clamp(64px,9vw,110px)] font-extrabold leading-none text-white drop-shadow-[0_6px_30px_rgba(0,0,0,0.5)]", 
                    low && "text-[#ff8a8a] blink")}>
                  ...
                  <div className="mx-auto mt-4 h-2 w-[240px] max-w-full overflow-hidden rounded-full bg-black/40">
                    <div className="h-full rounded-full bg-[#ffd76a] transition-all duration-1000 ease-linear"
                      style={{ 
                        width: `${(p.timeRemaining / ((p.currentBreak ? 
                          (t.structure.breaks.find((b) => b.afterLevel === p.currentLevel)?.duration ?? 10) : 
                          info.lv.duration) * 60)) * 100}%` 
                      }} />
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-3 text-center sm:text-left">
                  ...
                  {next && !p.currentBreak && (
                    <p className="col-span-3 pt-1 text-[11.5px] font-bold text-white/45">
                      Далее: {next.sb}/{next.bb}{next.ante ? ` · анте ${next.ante}` : ""} · {next.duration} мин
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="mt-6 grid grid-cols-3 gap-2 sm:grid-cols-6">
                <CtrlBtn 
                  label={p.timerPaused ? "Старт" : "Пауза"} 
                  icon={p.timerPaused ? <Play className="size-4.5" /> : <Pause className="size-4.5" />} 
                  onClick={handleTogglePause} 
                  gold 
                />
                <CtrlBtn label="Перерыв" icon={<Coffee className="size-4.5" />} onClick={handleStartBreak} />
                <CtrlBtn label="Пред. ур." icon={<SkipBack className="size-4.5" />} onClick={() => handleStepLevel(-1)} />
                <CtrlBtn label="След. ур." icon={<SkipForward className="size-4.5" />} onClick={() => handleStepLevel(1)} />
                <CtrlBtn label="−1 мин" icon={<Minus className="size-4.5" />} onClick={() => handleAddMinute(-1)} />
                <CtrlBtn label="+1 мин" icon={<Plus className="size-4.5" />} onClick={() => handleAddMinute(1)} />
              </div>
              <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
                <Btn variant="ghost" className="!border-white/15 !bg-black/25 !text-white hover:!bg-black/45" onClick={() => setModal("knock")}>
                  <Skull className="size-4.5 text-[#ff8a8a]" /> Выбивание
                </Btn>
                <Btn variant="ghost" className="!border-white/15 !bg-black/25 !text-white hover:!bg-black/45" onClick={() => setModal("bonus")}>
                  <Gift className="size-4.5 text-[#ffd76a]" /> Бонус
                </Btn>
                <Btn variant="danger" onClick={() => setModal("finish")} disabled={loading}>
                  <Flag className="size-4.5" /> Завершить турнир
                </Btn>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <Reveal delay={80}>
            <div className="panel relative overflow-hidden p-5">
              ...
              <p className="mt-1.5 text-[12.5px] font-semibold text-mut">
                фишек в игре · средний стек {playersIn ? fmtNum(Math.round(bank / playersIn)) : 0}
              </p>
              ...
              <div className="mt-3 rounded-xl border border-line bg-white/[0.03] p-3.5">
                ...
                <div className="flex items-center gap-2">
                  <input 
                    type="number" 
                    placeholder="Кол-во фишек" 
                    className="inp !min-h-[42px] num" 
                    value={wd} 
                    onChange={(e) => setWd(e.target.value)}
                    onKeyDown={(e) => { if (e.key === "Enter") doWithdraw(); }} 
                  />
                  <Btn onClick={doWithdraw} disabled={!(+wd > 0) || loading}>Вывести</Btn>
                </div>
                <p className="mt-2 text-[11.5px] font-semibold text-dim">
                  {withdrawn > 0 ? (
                    <>Выведено за турнир: <b className="num text-warn">−{fmtNum(withdrawn)}</b> · в банке {fmtNum(bank)}</>
                  ) : (
                    "Сумма вычитается из общего банка турнира"
                  )}
                </p>
              </div>
            </div>
                        ...
                        {lcUid === uidv && (
                          <div className="anim-pop mt-2 flex items-center gap-1.5 rounded-lg border border-(--acc-line) bg-(--acc-soft) p-1.5">
                            <input 
                              ...
                              type="number" 
                              placeholder="Кол-во фишек" 
                              className="inp !min-h-[38px] !rounded-lg !px-2.5 !py-1 num !text-[13px]"
                              value={lcChips} 
                              onChange={(e) => setLcChips(e.target.value)}
                              onKeyDown={(e) => { if (e.key === "Enter") doLastChance(uidv); }} 
                            />
                            <Btn size="sm" onClick={() => doLastChance(uidv)} disabled={loading}>
                              <Repeat className="size-4" /> Вернуть
                            </Btn>
                          </div>
                        )}
      ...
      <Modal open={modal === "knock"} onClose={() => setModal(null)} title="Выбивание игрока" subtitle="Фишки выбывшего переходят победителю раздач">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Btn variant="danger" onClick={doKnock} disabled={loading}>
              <Skull className="size-4" /> Выбить из турнира
            </Btn>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={modal === "bonus"} onClose={() => setModal(null)} title="Начислить бонус" subtitle="Фишки сразу добавляются к стеку игрока">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <Btn variant="ghost" onClick={() => setModal(null)}>Отмена</Btn>
            <Btn onClick={doBonus} disabled={loading}>
              <Gift className="size-4" /> Выдать бонус
            </Btn>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={modal === "finish"} onClose={() => setModal(null)} title="Завершить турнир?" w="max-w-xl"
        ...
        <div className="space-y-4">
          <div className="flex gap-3 rounded-xl border border-warn/30 bg-warn/10 p-4 text-[13px] font-semibold leading-relaxed text-warn">
            ...
            <span>
              Оставшиеся {playersIn} {plural(playersIn, "игрок", "игрока", "игроков")} будут распределены по местам согласно стекам, 
              выбывшие — по времени вылета. Очки, статистика, достижения и уведомления начислятся автоматически.
            </span>
          </div>
          ...
          <div className="flex justify-end gap-2">
            <Btn variant="ghost" onClick={() => setModal(null)}>Отмена</Btn>
            <Btn variant="danger" onClick={doFinish} disabled={loading}>
              <Flag className="size-4" /> Подвести итоги
            </Btn>
          </div>
        </div>
      </Modal>
...
function CtrlBtn({ label, icon, onClick, gold }: { label: string; icon: React.ReactNode; onClick: () => void; gold?: boolean }) {
  return (
    <button 
      onClick={onClick}
      className={cn(
        "flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-xl border px-2 py-2.5 text-[11px] font-extrabold uppercase tracking-wide transition-all duration-200 active:scale-95",
        gold ? "border-[#ffd76a]/40 bg-[#ffd76a]/15 text-[#ffd76a] hover:bg-[#ffd76a]/25" : 
        "border-white/12 bg-black/25 text-white/85 hover:bg-black/45 hover:border-white/25"
      )}
    >
      {icon}{label}
    </button>
